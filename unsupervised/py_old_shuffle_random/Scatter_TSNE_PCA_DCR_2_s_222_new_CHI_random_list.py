...
print (df0_CoVs_ORF_training.shape)
df0_CoVs_ORF_training = np.array (df0_CoVs_ORF_training)
random.shuffle(df0_CoVs_ORF_training)
df0_CoVs_ORF_training = pd.DataFrame(df0_CoVs_ORF_training)
#df0_CoVs_ORF_training.to_csv ('xxxxxxx.csv')
X = df0_CoVs_ORF_training.iloc[:, 1:-4]
#X1=X.iloc[:,0]
#print(X1)#全部组成性信息（去掉后面的密码子和标签列）
y = (df0_CoVs_ORF_training.iloc[:, -1]).tolist()#adaptation列，即0，1，2标签
print (X.shape)
...
SMOTE_train_x1 = SMOTE_train_x.iloc[:,0]
print(SMOTE_train_x1)
