...
#df0_CoVs_Artiodactyla['adaptation'] = [2]*df0_CoVs_Artiodactyla.shape[0]
#print (df0_CoVs_Artiodactyla.shape)
# 
#df0_CoVs_Rodent_Lagomorpha = df0_CoVs [(df0_CoVs ['host']=='Rodent_Lagomorpha')]
#print (df0_CoVs_Rodent_Lagomorpha.shape)
#df0_CoVs_Rodent_Lagomorpha['adaptation'] = [3]*df0_CoVs_Rodent_Lagomorpha.shape[0]
#print (df0_CoVs_Rodent_Lagomorpha.shape)
#
#df0_CoVs_Carnivora = df0_CoVs [(df0_CoVs ['host']=='Carnivora')]
#print (df0_CoVs_Carnivora.shape)
#df0_CoVs_Carnivora['adaptation'] = [4]*df0_CoVs_Carnivora.shape[0]
#print (df0_CoVs_Carnivora.shape)
# 
#df0_CoVs_Primates = df0_CoVs [(df0_CoVs ['host']=='Primates')].sample (frac = 0.17, random_state = 10)
#print (df0_CoVs_Primates.shape)
#df0_CoVs_Primates['adaptation'] = [5]*df0_CoVs_Primates.shape[0]
#print (df0_CoVs_Primates.shape)
# 
#df0_CoVs_all = pd.concat ([df0_CoVs_Chiroptera, df0_CoVs_Suiformes, df0_CoVs_Artiodactyla, df0_CoVs_Rodent_Lagomorpha, df0_CoVs_Carnivora, df0_CoVs_Primates], axis = 0)#连接参数产生的字符串
...
#
#
#df_CoVs_ORF = pd.read_csv ('../data/df_full_DCR_counting_S_ORF1ab_Coronaviridae(Organism)_26-32k_2564_CDS_S_host_2_SARS2.csv')
...
#df0_CoVs_ORF_training.to_csv ('df0_CoVs_ORF_training_shuffle.csv')

df0_CoVs_ORF_training_shuffle=pd.read_csv ('../data/df0_CoVs_ORF_training_shuffle.csv')
print (df0_CoVs_ORF_training_shuffle.shape)

df0_CoVs_Chiroptera=df0_CoVs_ORF_training_shuffle[(df0_CoVs_ORF_training_shuffle['host1']=='CHI')]
print (df0_CoVs_Chiroptera.shape)
df0_CoVs_Chiroptera['adaptation'] = [0]*df0_CoVs_Chiroptera.shape[0]

df0_CoVs_Suiformes=df0_CoVs_ORF_training_shuffle[(df0_CoVs_ORF_training_shuffle['host1']=='SUI')]
df0_CoVs_Suiformes['adaptation'] = [1]*df0_CoVs_Suiformes.shape[0]

df0_CoVs_Artiodactyla=df0_CoVs_ORF_training_shuffle[(df0_CoVs_ORF_training_shuffle['host1']=='ART')]
df0_CoVs_Artiodactyla['adaptation'] = [2]*df0_CoVs_Artiodactyla.shape[0]

df0_CoVs_Rodent_Lagomorpha=df0_CoVs_ORF_training_shuffle[(df0_CoVs_ORF_training_shuffle['host1']=='ROD_LAG')]
df0_CoVs_Rodent_Lagomorpha['adaptation'] = [3]*df0_CoVs_Rodent_Lagomorpha.shape[0]

df0_CoVs_Carnivora=df0_CoVs_ORF_training_shuffle[(df0_CoVs_ORF_training_shuffle['host1']=='CAR')]
df0_CoVs_Carnivora['adaptation'] = [4]*df0_CoVs_Carnivora.shape[0]

df0_CoVs_Primates=df0_CoVs_ORF_training_shuffle[(df0_CoVs_ORF_training_shuffle['host1']=='PRI')]
df0_CoVs_Primates['adaptation'] = [5]*df0_CoVs_Primates.shape[0]

df0_CoVs_ORF_training=pd.concat ([df0_CoVs_Chiroptera, df0_CoVs_Suiformes, df0_CoVs_Artiodactyla, df0_CoVs_Rodent_Lagomorpha, df0_CoVs_Carnivora, df0_CoVs_Primates], axis = 0)#连接参数产生的字符串
print (df0_CoVs_ORF_training.shape)

df0_CoVs_ORF_training['adaptation'] = df0_CoVs_ORF_training['adaptation'].tolist()

X = df0_CoVs_ORF_training.iloc[:, 3:-5]#全部组成性信息（去掉后面的密码子和标签列）
...
train_x, valid_x, train_y, valid_y = train_test_split(X, y, test_size=0.01,stratify=y, random_state=4)#X：要划分的样本特征集，y：要划分的样本结果，testsize：样本占比，种子
...
#            plt.ylim([-0.1,1.1])
            ...
            sns.set_style("white")
#             plt.legend(scatterpoints=1)
        plt.savefig('sns_scatterplot_tSNE_PCA_S_' + set_name + '_sampling200_test.png', dpi = 300, bbox_inches = 'tight')
