...
df0_CoVs_Chiroptera0 = df0_CoVs1 [(df0_CoVs1 ['host']=='Chiroptera')].sample (frac = 0.0893, random_state = 10)#112
print (df0_CoVs_Chiroptera0.shape)
df0_CoVs_Chiroptera0['adaptation'] = [0]*df0_CoVs_Chiroptera0.shape[0]
print (df0_CoVs_Chiroptera0.shape)

df0_CoVs_Chiroptera1 = df0_CoVs1 [(df0_CoVs1 ['host']=='Chiroptera_1')].sample (frac = 0.0529, random_state = 10)#189
...
df0_CoVs_Artiodactyla = df0_CoVs [(df0_CoVs ['host']=='Artiodactyla')].sample (frac = 0.0345, random_state = 10)#579
print (df0_CoVs_Artiodactyla.shape)
df0_CoVs_Artiodactyla['adaptation'] = [1]*df0_CoVs_Artiodactyla.shape[0]
print (df0_CoVs_Artiodactyla.shape)

df0_CoVs_Rodent_Lagomorpha = df0_CoVs [(df0_CoVs ['host']=='Rodent_Lagomorpha')].sample (frac = 0.4167, random_state = 10)#48
print (df0_CoVs_Rodent_Lagomorpha.shape)
df0_CoVs_Rodent_Lagomorpha['adaptation'] = [2]*df0_CoVs_Rodent_Lagomorpha.shape[0]
print (df0_CoVs_Rodent_Lagomorpha.shape)

df0_CoVs_Carnivora = df0_CoVs [(df0_CoVs ['host']=='Carnivora')].sample (frac = 0.2532, random_state = 10)#79
print (df0_CoVs_Carnivora.shape)
df0_CoVs_Carnivora['adaptation'] = [3]*df0_CoVs_Carnivora.shape[0]
print (df0_CoVs_Carnivora.shape)

df0_CoVs_Primates = df0_CoVs [(df0_CoVs ['host']=='Primates')].sample (frac = 0.0412, random_state = 10)#485
...
df0_CoVs_sui_all = pd.concat ([df0_CoVs_Chiroptera, df0_CoVs_Artiodactyla, df0_CoVs_Rodent_Lagomorpha, df0_CoVs_Carnivora, df0_CoVs_Primates, df0_CoVs_SADS, df0_CoVs_PEDV, df0_CoVs_PHEV, df0_CoVs_TGEV, df0_CoVs_PDCoV], axis = 0)#连接参数产生的字符串
print (df0_CoVs_sui_all.shape)


id_list = df0_CoVs_sui_all['id'].tolist()
df0_CoVs_sui_all['id'] = df0_CoVs_sui_all['id'].astype('category')#改变数据类型？
df0_CoVs_sui_all['id'].cat.reorder_categories(id_list, inplace=True)#？？？
df0_CoVs_sui_all.sort_values('id', inplace=True)#将数据集依照某个字段的数据排序，上三行在对accession排序，确保后面加adaptation列时顺序一致


df_CoVs_ORF_sui_all = pd.read_csv ('../data/df_full_DCR_counting_ORF1ab.csv')
print (df_CoVs_ORF_sui_all.shape)
df0_CoVs_ORF_training_sui_all = df_CoVs_ORF_sui_all [df_CoVs_ORF_sui_all['accession'].isin(id_list)]#筛选accession列中值在id_list中的行
print (df0_CoVs_ORF_training_sui_all.shape)

df0_CoVs_ORF_training_sui_all['accession'] = df0_CoVs_ORF_training_sui_all['accession'].astype('category')
df0_CoVs_ORF_training_sui_all['accession'].cat.reorder_categories(id_list, inplace=True)
df0_CoVs_ORF_training_sui_all.sort_values('accession', inplace=True)
print (df0_CoVs_ORF_training_sui_all.shape)
df0_CoVs_ORF_training_sui_all['adaptation'] = df0_CoVs_sui_all['adaptation'].tolist()#加一列adaptation
print (df0_CoVs_ORF_training_sui_all.shape)
# df0_CoVs_ORF_training_sui_all = shuffle(df0_CoVs_ORF_training_sui_all)

df0_CoVs_ORF_training_sui_all.to_csv ('Hier_counting_post_sampling_ORF1ab_0.csv')



# df0_CoVs_ORF_training_sui = pd.read_csv ('shuffle_save_sui.csv')
...
#             sns.set_style("white")
# #             plt.legend(scatterpoints=1)
#         plt.savefig('sns_scatterplot_tSNE_PCA_ORF1ab_' + set_name + '_sampling200_test.png', dpi = 300, bbox_inches = 'tight')
